...
// multiple, and the corpse warning — and, post-retirement (issue #105), no
// longer implies a "system's stake" that no auto-buy places.
func TestSnipeDeepText(t *testing.T) {
	...
	if strings.Contains(got, "system's stake") {
		t.Errorf("snipeDeepText must not imply a system stake (message truth) — the deep tier is alert-only:\n%s", got)
	}
}

// TestNotifySnipeDeepCrashDMHonest (issue #105): the alert-only Deep Crash DM
// carries the honest no-auto-buy line and must NOT claim or imply a buy — no
// "Auto-buy skipped" (there is no auto-buy to skip) and no "auto-bought".
func TestNotifySnipeDeepCrashDMHonest(t *testing.T) {
	t.Parallel()
	...
	if got := h.buys.count(); got != 0 {
		t.Fatalf("buy calls = %d, want 0 (deep tier is alert-only)", got)
	}
	sent := h.tg.sentAt(t, 0)
	if !strings.Contains(sent.text, "alert-only") || !strings.Contains(sent.text, "no auto-buy at this depth") {
		t.Errorf("deep DM missing the honest alert-only line:\n%s", sent.text)
	}
	for _, forbidden := range []string{"Auto-buy skipped", "auto-bought"} {
		if strings.Contains(sent.text, forbidden) {
			t.Errorf("deep DM must not contain %q (message truth):\n%s", forbidden, sent.text)
		}
	}
	// The MAIN pool is never touched by a deep fire.
	if _, ok := h.bot.snipeSpend.reserve(7, snipeAutoBuyDailyCapUSD); !ok {
		t.Error("deep alert consumed the main pool")
	}
}
