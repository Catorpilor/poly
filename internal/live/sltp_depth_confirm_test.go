...
// partial-depth (depth < fired size) asymmetry between TP and SL.
//
// The fired size is a fixed 100 shares; `depth` is scripted relative to it.
func TestSLTPMonitor_ConfirmFire_Decision(t *testing.T) {
	...
	cases := []struct {
		...
		// --- fail-open: no local book (ok=false) always fires ---
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			feed := newFakeFeed()
			feed.setVWAP("TOK", tc.vwap, tc.depth, tc.ok)
			m := NewSLTPMonitor(newFakeStore(), feed, &fakeExecutor{}, &fakeNotifier{}, nil)
...
// TestSLTPMonitor_ConfirmFire_CooldownAndReArm proves a refused fire re-arms
// and, after the 5s per-arm cooldown, fires when the book is genuinely crossed
// — and that back-to-back attempts inside the cooldown are suppressed before
// any book read.
func TestSLTPMonitor_ConfirmFire_CooldownAndReArm(t *testing.T) {
	t.Parallel()
	feed := newFakeFeed()
	clock := newFakeClock()
	m := NewSLTPMonitor(newFakeStore(), feed, &fakeExecutor{}, &fakeNotifier{}, nil)
	m.now = clock.now
	arm := &database.SLTPArm{ID: 7, TelegramID: 3, TokenID: "RC"}

	// Phantom: VWAP 0.10 under the 0.20 TP threshold → refuse.
	feed.setVWAP("RC", 0.10, 1000, true)
	if m.confirmFire(arm, "TP", 0.24, 0.20, 100) {
		t.Fatal("phantom fire should be refused")
	}
	if n := feed.vwapCallCount(); n != 1 {
		t.Fatalf("expected 1 VWAP read, got %d", n)
	}

	// Retry inside the 5s cooldown: suppressed BEFORE any VWAP read.
	if m.confirmFire(arm, "TP", 0.24, 0.20, 100) {
		t.Fatal("retry within cooldown should stay refused")
	}
	if n := feed.vwapCallCount(); n != 1 {
		t.Fatalf("cooldown must suppress the confirm attempt; got %d reads", n)
	}

	// After the cooldown the book is genuinely crossed → fire.
	clock.advance(5 * time.Second)
	feed.setVWAP("RC", 0.22, 1000, true)
	if !m.confirmFire(arm, "TP", 0.24, 0.20, 100) {
		t.Fatal("after cooldown a genuinely-crossed book must fire")
	}
	if n := feed.vwapCallCount(); n != 2 {
		t.Fatalf("expected the post-cooldown attempt to read VWAP, got %d", n)
	}
}
...
func TestSLTPMonitor_ConfirmFire_RefusedLog(t *testing.T) {
	...
	feed := newFakeFeed()
	feed.setVWAP("LG", 0.21, 1000, true)
	m := NewSLTPMonitor(newFakeStore(), feed, &fakeExecutor{}, &fakeNotifier{}, nil)
...
// --- integration: the wiring actually gates ExecuteSell ---

func TestSLTPMonitor_TP_PhantomRefusedThenReArmsAndFires(t *testing.T) {
	t.Parallel()
	store := newFakeStore()
	// HWM == avg keeps the SL dormant so only the TP branch is exercised.
	store.seed(&database.SLTPArm{ID: 20, TelegramID: 8, TokenID: "PT", AvgPrice: 0.10, SharesAtArm: 100,
		HighWaterMark: 0.10, TickSize: 0.01, TPArmed: true, SLArmed: true})
	feed := newFakeFeed()
	exec := &fakeExecutor{}
	notif := &fakeNotifier{}
	clock := newFakeClock()
	...
	// Phantom: bid 0.24 crosses the 0.20 trigger, but the book VWAP is 0.09.
	feed.setVWAP("PT", 0.09, 1000, true)
	feed.setBid("PT", 0.24)
	feed.emit("PT")
	waitFor(t, func() bool { return feed.vwapCallCount() >= 1 })

	exec.mu.Lock()
	calls := len(exec.calls)
	exec.mu.Unlock()
	if calls != 0 {
		t.Fatalf("phantom TP must not sell, got %d calls", calls)
	}
	if store.armedCount("PT") != 1 || !store.byToken["PT"][0].TPArmed {
		t.Fatal("TP must stay armed after a refusal (re-arm, not consume)")
	}

	// The book becomes genuinely crossed; after the cooldown the TP fires.
	clock.advance(5 * time.Second)
	feed.setVWAP("PT", 0.21, 1000, true)
	feed.emit("PT")
	waitFor(t, func() bool {
		notif.mu.Lock()
...
func TestSLTPMonitor_Ceiling_PhantomRefused_GenuineFires_FailOpen(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.ok {
				feed.setVWAP("CT", tc.vwap, 100000, true)
			}
			...
			if tc.wantFire {
				...
				// Give the goroutine time to (not) act, then assert no sell.
				waitFor(t, func() bool { return feed.vwapCallCount() >= 1 })
				exec.mu.Lock()
				calls := len(exec.calls)
				exec.mu.Unlock()
				if calls != 0 {
					t.Fatalf("phantom ceiling must not sell, got %d calls", calls)
				}
...
func TestSLTPMonitor_SL_PhantomLowRefused(t *testing.T) {
	...
	feed.setVWAP("SX", 0.153, 1000, true)
	feed.setBid("SX", 0.06)
	feed.emit("SX")
	...
	exec.mu.Unlock()
	if calls != 0 {
		t.Fatalf("phantom-low SL must not sell, got %d calls", calls)
	}
...
func TestSLTPMonitor_SL_GenuineCollapseFires(t *testing.T) {
	...
	feed.setVWAP("SY", 0.05, 1000, true)
	feed.setBid("SY", 0.06)
	feed.emit("SY")
